...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» https://graph.org/file/05cc2603544685a5f6592.jpg")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🤖DARK IS ALIVE BABY__\n» `{mp} MS`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        sudousers = getenv("SUDO_USERS", default=5695560295)

        ok = await event.reply(f"» __DARK ADDING USER IN SUDOLIST...__")
        target = ""
        ...
        await event.reply("» BABE, ᴏɴʟʏ ᴏᴡɴᴇʀ ᴄᴀɴ ᴀᴄᴄᴇꜱꜱ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ.")
